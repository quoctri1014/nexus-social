...
  if (!token) { window.location.href = "/index.index.html"; return; } // Sửa lỗi href
...
  // 12. CALL & VIDEO CALL (BUTTON CLICK HANDLERS)
...
  
  // FIX CRASH: Bảo vệ listener của nút đóng modal group
  const closeGroupModalBtn = document.getElementById("close-group-modal");
  if(closeGroupModalBtn) closeGroupModalBtn.addEventListener("click", () => document.getElementById("group-modal").classList.add("hidden"));
});
```Tôi đã kiểm tra lại file `main.js` của bạn. Lỗi `TypeError: Cannot read properties of null` mà bạn thấy trong ảnh là do file `main.js` và `webrtc.js` đều cố gắng truy cập các phần tử HTML mà chúng chưa tìm thấy.

Tôi đã thêm các biến và kiểm tra `if(element)` vào file `main.js` để bảo vệ các listener, bao gồm cả nút đóng modal group, là nơi thường xuyên gây crash.

Dưới đây là file `main.js` đã được hoàn thiện và bảo vệ khỏi lỗi `null` (loại lỗi mà bạn đã chỉ ra):

### File `main.js` (Hoàn chỉnh và Đã Bảo vệ Listener)
...
  // 3. DOM ELEMENTS (Đã khai báo tất cả các Modal DOMs)
  const chatContainer = document.getElementById("main-container");
  const messagesContainer = document.getElementById("messages");
  const messageInput = document.getElementById("message-input");
  const sendBtn = document.getElementById("send-btn");
