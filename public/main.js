document.addEventListener("DOMContentLoaded", () => {
  ...
  if (logoutBtn) logoutBtn.addEventListener("click", () => { localStorage.removeItem("token"); window.location.href = "/index.html"; });

  document.body.setAttribute("data-theme", localStorage.getItem("theme") || "dark");
  function applyBg(val) {
    if (!val) return; val = val.trim();
    const isImage = val.startsWith("http") || val.startsWith("https") || val.startsWith("data:image") || val.startsWith("url");
    if (isImage) {
  ...
  window.socket.on("userList", (users) => {
    if (!userListDiv) return; userListDiv.innerHTML = "";
    users.forEach((u) => {
      if (u.id === window.myUserId) return;
      const div = document.createElement("div");
      div.className = `user-item ${window.currentChatContext.id === u.id ? "active" : ""}`;
      div.innerHTML = `<div class="user-avatar">${getAvatarHtml(u)}<div class="status-dot ${u.online ? "online" : ""}"></div></div><div class="user-info"><div class="user-name">${u.nickname || u.username}</div><div class="user-preview">${u.id === AI_BOT_ID ? "Luôn sẵn sàng" : u.online ? "Online" : "Offline"}</div></div>`;
  ...
  window.appendMessage = function (msg, shouldScroll = true) {
    ...
    const isSystemMsg = msg.isSystem || msg.content.includes("Cuộc gọi");
    ...
    if (msg.ttl) {
      ...
      const timeLeft = (createdAt + msg.ttl) - Date.now();
      if (timeLeft > 0) setTimeout(() => { div.style.opacity = "0"; setTimeout(() => div.remove(), 500); }, timeLeft);
      else return;
    }

    let delBtn = "";
    if (msg.senderId === window.myUserId && !isSystemMsg)
  ...
  if (chatForm) chatForm.addEventListener("submit", (e) => {
      e.preventDefault(); const val = messageInput.value.trim();
      ...
      window.socket.emit("privateMessage", data); messageInput.value = "";
  });

  if (messageInput) messageInput.addEventListener("input", (e) => {
      if (e.target.value.trim()) { sendBtn.classList.remove("hidden"); heartBtn.classList.add("hidden"); } else { sendBtn.classList.add("hidden"); heartBtn.classList.remove("hidden"); }
  });

  if (attachBtn && hiddenFileInput) {
    attachBtn.addEventListener("click", () => { if (!window.currentChatContext.id) return alert("Chọn người chat trước!"); hiddenFileInput.click(); });
    hiddenFileInput.addEventListener("change", async (e) => {
      ...
      try {
        ...
        uploadedFiles.forEach((file) => { window.socket.emit("privateMessage", { recipientId: window.currentChatContext.id, content: JSON.stringify({ type: file.type, url: file.url, name: file.name }) }); });
      } catch (err) { alert("Lỗi upload"); } finally { messageInput.placeholder = "Nhập tin nhắn..."; hiddenFileInput.value = ""; }
    });
  }
      ...
      if (!isRecording) {
        try {
          ...
          mediaRecorder.onstop = async () => {
            ...
            const formData = new FormData(); formData.append("files", blob, `voice_${Date.now()}.webm`);
            voiceBtn.classList.remove("recording"); messageInput.placeholder = "Đang gửi...";
            try {
              const res = await fetch("/api/upload", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: formData });
              const files = await res.json();
              if (files.length) window.socket.emit("privateMessage", { recipientId: window.currentChatContext.id, content: JSON.stringify({ type: "audio", url: files[0].url }) });
            } catch (err) { alert("Lỗi gửi voice"); }
            stream.getTracks().forEach((t) => t.stop()); messageInput.placeholder = "Nhập tin nhắn...";
          };
          mediaRecorder.start(); isRecording = true; voiceBtn.classList.add("recording"); messageInput.placeholder = "Đang ghi âm...";
        } catch (e) { alert("Không tìm thấy Micro!"); }
      } else { mediaRecorder.stop(); isRecording = false; }
    });
  }

  if (callBtn) callBtn.addEventListener("click", () => { if (!window.currentChatContext.id) return; window.socket.emit("callOffer", { recipientId: window.currentChatContext.id, type: "audio" }); });
  if (videoBtn) videoBtn.addEventListener("click", () => { if (!window.currentChatContext.id) return; window.socket.emit("callOffer", { recipientId: window.currentChatContext.id, type: "video" }); });

  const emojiBtn = document.getElementById("emoji-trigger"); const emojiPicker = document.getElementById("emoji-picker");
  if (emojiBtn && emojiPicker) {
    emojiBtn.addEventListener("click", (e) => { e.stopPropagation(); emojiPicker.classList.toggle("hidden"); });
    document.querySelectorAll(".emoji-grid span").forEach((span) => { span.addEventListener("click", () => { messageInput.value += span.innerText; messageInput.focus(); emojiPicker.classList.add("hidden"); sendBtn.classList.remove("hidden"); heartBtn.classList.add("hidden"); }); });
    document.addEventListener("click", (e) => { if (!emojiPicker.contains(e.target) && e.target !== emojiBtn) emojiPicker.classList.add("hidden"); });
  }

  const setBtn = document.getElementById("chat-settings-btn"); const closeBgBtn = document.getElementById("close-bg-modal"); const saveBgBtn = document.getElementById("save-bg-btn"); const bgModal = document.getElementById("bg-modal"); const bgInput = document.getElementById("bg-url-input");
  ...
  document.querySelectorAll(".bg-preset").forEach((preset) => { preset.addEventListener("click", () => { applyBg(preset.getAttribute("data-bg")); bgModal.classList.add("hidden"); }); });
  if (saveBgBtn) saveBgBtn.addEventListener("click", () => { const url = bgInput.value.trim(); if (url) { applyBg(url); bgModal.classList.add("hidden"); bgInput.value = ""; } else alert("Nhập link!"); });
  ...
  function showHeartAnimation() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      heart.style.left = Math.random() * 100 + "%"; heart.style.animationDuration = 2 + Math.random() * 3 + "s";
      container.appendChild(heart); setTimeout(() => heart.remove(), 4000);
    }
  }

  window.deleteMessage = (id) => { if (confirm("Thu hồi?")) window.socket.emit("deleteMessage", { messageId: id, recipientId: window.currentChatContext.id }); };
  window.socket.on("messageDeleted", ({ messageId }) => { const el = document.getElementById(`msg-${messageId}`); if (el) el.remove(); });

  if (secretBtn) secretBtn.addEventListener("click", () => { isSecretMode = !isSecretMode; if (isSecretMode) { secretBtn.classList.add("active-secret"); secretBtn.style.color = "#ef4444"; messageInput.placeholder = "Chế độ tự hủy (10s)..."; } else { secretBtn.classList.remove("active-secret"); secretBtn.style.color = ""; messageInput.placeholder = "Nhập tin nhắn..."; } });

  const backBtn = document.getElementById("mobile-back-btn"); if (backBtn) backBtn.addEventListener("click", () => { chatContainer.classList.remove("mobile-active"); window.currentChatContext = { id: null }; });
  const createGroupBtn = document.getElementById("create-group-btn"); if (createGroupBtn) createGroupBtn.addEventListener("click", () => document.getElementById("group-modal").classList.remove("hidden"));
