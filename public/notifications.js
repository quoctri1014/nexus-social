document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadNotificationsPage() {
    ...
    try {
      ...
      notifs.forEach((n) => {
        const iconHtml = n.type === "request" ? `<div class="notif-icon icon-req"><i class="fas fa-user-plus"></i></div>` : `<div class="notif-icon icon-post"><i class="fas fa-newspaper"></i></div>`;
        const contentText = n.type === "request" ? `<b>${n.nickname || n.username}</b> đã gửi cho bạn một lời mời kết bạn.` : `<b>${n.nickname || n.username}</b> vừa đăng một bài viết mới.`;
        const linkAction = n.type === "request" ? "window.location.href='/friends.html'" : "window.location.href='/home.html'";

        const html = `
            <div class="notif-item-full" onclick="${linkAction}">
                <div class="notif-img-full"><img src="${getAvatarDisplay(n)}"></div>
    ...
    } catch (e) { container.innerHTML = `<div class="empty-state">Lỗi tải thông báo.</div>`; }
  }
