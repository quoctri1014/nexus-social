...
// --- CLOUDINARY CONFIG (QUAN TRỌNG) ---
// Kiểm tra xem biến môi trường đã có chưa
if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY || !process.env.CLOUDINARY_API_SECRET) {
    console.error("❌ LỖI: Thiếu cấu hình Cloudinary trên Render! Hãy kiểm tra lại Environment Variables.");
}
...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    // Tự động xác định loại resource (image, video, raw)
    let resourceType = 'auto';
    if (file.mimetype.startsWith('audio')) resourceType = 'video'; // Cloudinary xử lý audio như video
    
    return {
      folder: 'nexus_uploads',
      resource_type: resourceType,
      public_id: `${Date.now()}-${file.originalname.split('.')[0]}`, // Đặt tên file unique
    };
  },
});
...
app.post("/api/upload", upload.array("files", 5), (req, res) => {
  ...
  const files = req.files.map(f => ({
    ...
    name: f.originalname,
    url: f.path // Cloudinary trả về link trực tiếp ở đây
  }));
  
  console.log("Upload success:", files); // Log để kiểm tra
  res.json(files);
});
