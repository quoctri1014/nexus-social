...
// ================= SOCKET.IO & AI LOGIC (FIXED) =================

// Hàm xử lý AI KHÔNG dùng thư viện (Dùng fetch trực tiếp)
async function handleAIChat(msg, uid, socket) {
  ...
  try {
    // 1. Gọi trực tiếp API của Google (Bỏ qua lỗi thư viện)
    ...
    socket.emit("newMessage", { senderId: AI_BOT_ID, content: "AI đang gặp sự cố kết nối. Thử lại sau nhé!", createdAt: new Date() });
  }
}
