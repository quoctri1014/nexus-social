...
// Hàm gọi Google Gemini API v1 với model chính xác
async function callGeminiAPI(text) {
  // Sử dụng gemini-2.0-flash hoặc gemini-1.5-flash với v1 API
...
async function handleAIChat(msg, uid, socket) {
  ...
  if (data && data.candidates && data.candidates.length > 0) {
    ...
    socket.emit("newMessage", {
      senderId: AI_BOT_ID,
      content: "Hệ thống AI đang quá tải. Vui lòng kiểm tra lại Key hoặc thử lại sau.",
      createdAt: new Date(),
    });
